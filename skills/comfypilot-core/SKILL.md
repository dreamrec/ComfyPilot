...
3. **Discover or build**
   - Search the library first: `comfy_search_techniques` for saved patterns, or `comfy_list_blueprints` for blueprints (13 bundled; pass `source="native"` to read ComfyUI's v0.3.67+ published subgraphs).
...
- `comfy://api/openapi` - ComfyUI OpenAPI 3.1 spec (v0.20.0+)
...
- `comfy://docs/{node_class}` - Embedded node documentation (v0.3.68+ with object_info description fallback)
